...
|---|---|---|
| Agente | **Codex CLI**, dirigido por este manual y por la skill de `.codex/skills/` | Recolecta, estructura y escribe los snapshots. |
...
cada mañana. Una tarea que se repite dos veces con los mismos pasos es
...
- **Red, limitada a los dominios del catálogo de fuentes.** Es lo que necesita
  la fase *Buscar*. Ninguna otra fase pide red, y un dominio que no está en el
  catálogo no se visita: si aparece uno nuevo, se agrega primero a
...
2. Invoca la skill `recolectar-noticias-ia` con la fecha de hoy. La skill pide
   la aprobación de red antes de *Buscar* y la de escritura antes de *Guardar*.
...
10. Hay un commit `data:` que contiene ese archivo y nada más.
